...
/* The config-announce console_field byte (cfea[19]) gates the box's drivable
 * rate CLASS.
 * 0x00 = V-Mixer (M-200 / M-300), capped at 44.1/48 kHz; 0x01 = OHRCA
 * (M-5000), which alone
 * reaches 96 kHz. The box FOLLOWS the byte the master emits, so driving a box
 * at 96 kHz
 ...
 * master's choice
 * (PACE_IS_THE_MASTERS); console_field is the wire byte by which the master
 * DECLARES the
 * class, and the family label (V-Mixer / OHRCA) is only its name. What is
 * independent of a
 * mixer MODEL is a bare clock number; what gates the class is this one
 * announced byte.
 * 44.1 kHz has NO distinct value — the field is binary — so it maps to 0x00
 * and the box
 ...
 * between the two,
 * the master byte the only differing field. LOGIC: an M-5000 running its own
 * box at 48 kHz
 * must emit 0x00, so the byte is the rate class, not merely the console
 * generation. Operator
 * ruling (2026-08-27): only OHRCA drives 96 kHz; V-Mixer is 48 kHz-limited
 * (M-300 @ 96 k
 * impossible). The 44.1 kHz mapping is DERIVED from the field being binary,
 * NOT wire-
 ...
 * timestamps puts
 * NOTHING in the 3675 band, while cleanly separating the 48k and 96k files.
 * The earlier
 ...
 * ]
 */
#define REAC_HEADAMP_BASE_FROM_CONFIG_BYTE7 True
 ...
 * ]
 */
#define REAC_HEADAMP_BASE_IS_CHASSIS_NOT_GRANT True
