...
instances:
  ...
  console_field_gates_rate:
    ...
    doc: |
      ...
      the master's choice
      (PACE_IS_THE_MASTERS); console_field and `revision` are the two wire
      fields by which the
      master DECLARES and then RECORDS the class, and the family label
      (V-Mixer / OHRCA) is
      ...
      between the two,
      the master byte the only differing field on that box (S-0808, fw
      1.003).
      CORRECTED (2026-08-29): that confirmation was generalised to "the box"
      and is not
      universal. An S-1608 (fw 2.200) held 48 kHz through a runtime
      assertion, a daemon cold
      boot at 96 kHz and a power-cycle into an already-running 96 kHz
      stream, all with
      console_field=0x01 — ten 96 kHz windows all measured WIRE PACE
      MISMATCH at 4000 pps.
      Making the scene's `revision` agree (see SCENE_REVISION_OFF) moved it
      to 96 kHz
      immediately and cleanly, and pinning `revision` made the box LATCH:
      neither a console
      assertion nor a daemon cold boot at 48 kHz moved it back, because the
      value never
  ...
  identity_addr_reac_version:
    ...
    doc: |
      ...
      console prints it `major.minorPP`, the patch zero-padded to two
      digits.
      ...
      other: the S-1608 runs firmware 2.200 and speaks REAC 2.302.
        [EVIDENCED (corpus + console display) — 274 replies, 5 distinct
        boxes,
  ...
  port_slot_in_split:
    ...
    doc: |
      ...
      Captured only on that chassis (MAC 00:40:ab:c4:25:80); a code this
      decoder used to REFUSE THE WHOLE TABLE on, which kept a fully
      declaring box off the graph for minutes (operator ruling 2026-09-17,
      "we should be able to enrol any stage box"). WHAT SEPARATES 0x00 FROM
      0x02 IS NOT DECIDED — one chassis, one capture; a splitter's inputs
      may be marked apart from a head-amp-owned input, or this may be
      nothing but an alternate spelling of `analog_input`. [EVIDENCED
  ...
  port_slot_unknown_cost:
    ...
    doc: |
      ...
      OTHERS — the box enrols at the width this decoder can read, and the
      unknown group is reported (`reac_ports_unknown`), never guessed into
      either role. No such code has been captured; this is the refusal rule
  ...
  port_slot_in_split_headamp_base:
    ...
    doc: |
      ...
      board_config_code * 0x10`. An M-200 has been observed addressing this
      box's preamps at CH 0x20–0x27 instead — a rig check, not a capture,
      and not yet reconciled with the strap law. Do not derive a base for a
      0x00-coded box from the strap alone until this is resolved. [OPEN (rig
      observation, unreproduced in a capture).]
  enroll_group_in:
    value: 0x41
    doc: |
